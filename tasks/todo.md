...
- [x] サブエージェントを使って slice ごとの承認候補をレビューし、`parallel_review_36000_final` に freeze する
- [x] `approved_review_candidates.tsv` / `approved_seed.csv` を retained slice から再構築し、36000 batch を反映する
- [x] `apply_review_batch.py` で `assets/words_core.jsonl` へ反映する
- [x] 新規追加帯の gloss と `distractor_group` ドリフトを監査し、`tabulate` / `unanticipated` / `unexpired` / `waylay` / `bleary` / `buss` / `cartwheel` / `creosote` / `tartar` / `teleportation` / `travelogue` / `tweezer` / `alarum` / `foghorn` / `lovebird` などを補正する
- [x] `go test ./...`、`go build ./...`、`validate --embedded-core`、fresh data dir の `stats` → `doctor` で整合性を検証する

# 2026-04-19 語彙追加 35000 seed batch

- [x] `parallel_review_35000` を `34001-35000` の範囲で作成する
...
- [x] `parallel_review_34000` を `33001-34000` の範囲で作成する
- [x] サブエージェントを使って slice ごとの承認候補をレビューし、`parallel_review_34000_final` に freeze する
- [x] `approved_review_candidates.tsv` / `approved_seed.csv` に 34000 batch をマージする
- [x] `apply_review_batch.py` で `assets/words_core.jsonl` へ反映する
- [x] 新規追加帯の gloss と `distractor_group` ドリフトを監査し、`admittance` / `aftershock` / `blare` / `brouhaha` / `curtsy` を group 補正し、`enclose` / `flask` の gloss を補正する
- [x] `go test ./...`、`go build ./...`、`validate --embedded-core`、fresh data dir の `stats` → `doctor` で整合性を検証する

# 2026-04-19 語彙追加 33000 seed batch

- [x] `parallel_review_33000` を `32001-33000` の範囲で作成する
- [x] サブエージェントを使って slice ごとの承認候補をレビューし、`parallel_review_33000_final` に freeze する
- [x] `approved_review_candidates.tsv` / `approved_seed.csv` に 33000 batch をマージする
- [x] `apply_review_batch.py` で `assets/words_core.jsonl` へ反映する
- [x] 新規追加帯の gloss と `distractor_group` ドリフトを監査し、`meanness` / `nonviolence` / `peacefulness` / `snobbery` / `vagueness` / `viciousness` / `wildness` / `dreamworld` / `gaiety` を `abstract-noun` へ補正する
- [x] `go test ./...`、`go build ./...`、`validate --embedded-core`、fresh data dir の `stats` → `doctor` で整合性を検証する

# 2026-04-19 語彙追加 32000 seed batch

- [x] `parallel_review_32000` を `31001-32000` の範囲で作成する
- [x] サブエージェントを使って slice ごとの承認候補をレビューする
- [x] `approved_review_candidates.tsv` / `approved_seed.csv` に 32000 batch をマージする
- [x] `apply_review_batch.py` で `assets/words_core.jsonl` へ反映する
- [x] 新規追加帯の adverb 代表訳と `distractor_group` ドリフトを監査し、`accumulator` を除外したうえで `crisscross` / `colloquial` / 人名詞分類ずれなどを補正する
- [x] `go test ./...`、`validate --embedded-core`、fresh data dir の `stats` → `doctor` で整合性を検証する

# 2026-04-19 語彙追加 31000 seed batch

- [x] `parallel_review_31000` を `30001-31000` の範囲で作成する
- [x] サブエージェントを使って slice ごとの承認候補をレビューする
- [x] `approved_review_candidates.tsv` / `approved_seed.csv` に 31000 batch をマージする
- [x] `apply_review_batch.py` で `assets/words_core.jsonl` へ反映する
- [x] 新規追加帯の `verb` 代表訳と `distractor_group` ドリフトを監査し、`upriver` / `grouch` を補正する
- [x] `go test ./...`、`validate --embedded-core`、fresh data dir の `stats` → `doctor` で整合性を検証する
